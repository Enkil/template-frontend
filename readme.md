...
Обязательным условием для корректнйо работы является наличие двух файлов изображений для одной иконки (например, html5.png и html5-2x.png).  
...
При выполнении задчи запускается локальный веб-сервер BrowserSync и открыватся index.html проекта.  
...
Рекомендуется изменять пути и целевые файлы только через этот раздел, при необходимости его дополняя новыми переменными.
